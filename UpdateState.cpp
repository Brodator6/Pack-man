...
static void UpdateState(Player *player, std::vector<std::vector<TileData>> &level, int rows, int columns){
    ...
    if(player->movement[SDL_SCANCODE_DOWN] == true){
        ...
        player->SetPosition(player->GetPositionX() + ((IsWalkable(level[player->GetPositionY()][(player->GetPositionX() + 1) % columns])))  - ((player->GetPositionX() >= columns - 1) * columns),
        player->GetPositionY());
        ...
        player->SetPosition(player->GetPositionX() - ((IsWalkable(level[player->GetPositionY()][(player->GetPositionX() - 1 + columns) % columns])))  + ((player->GetPositionX() <= 0) * columns),
        player->GetPositionY());
    }
